import { PROFILE } from '../data/portfolio.js';

const BUILDS = [
  'Multi-vendor e-commerce platform',
  'Role-based dashboards',
  'Authentication',
  'Supabase + PostgreSQL',
];

export default function Hero() {
  return (
    <section id="home" className="relative overflow-hidden">
      <div className="bg-grid absolute inset-0" aria-hidden="true" />
      <div
        className="absolute -top-32 right-[-10%] h-96 w-96 rounded-full bg-accent/10 blur-3xl"
        aria-hidden="true"
      />
      <div
        className="absolute bottom-[-20%] left-[-10%] h-80 w-80 rounded-full bg-white/5 blur-3xl"
        aria-hidden="true"
      />

      <div className="relative mx-auto flex min-h-screen max-w-6xl flex-col justify-center px-5 pb-16 pt-28 sm:px-8">
        <p className="mb-5 font-mono text-sm text-muted">
          <span className="text-accent">$</span> whoami
          <span className="text-muted/60"> — front-end web developer</span>
        </p>

        <h1 className="max-w-3xl text-4xl font-bold leading-tight tracking-tight text-paper sm:text-6xl lg:text-7xl">
          Hi, I'm Jenny <span aria-hidden="true">👋</span>
          <span className="mt-3 block bg-gradient-to-r from-paper via-white/80 to-muted bg-clip-text text-transparent">
            Front-End Web Developer
          </span>
        </h1>

        <p className="mt-6 max-w-xl text-lg leading-relaxed text-muted">
          {PROFILE.heroStatement}
        </p>
        <p className="mt-3 max-w-xl text-base leading-relaxed text-muted/80">
          {PROFILE.secondaryStatement}
        </p>

        <div className="mt-10 flex flex-wrap items-center gap-4">
          <a
            href="#projects"
            className="rounded-lg bg-accent px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-[#c93c41]"
          >
            View My Work
          </a>
          <a
            href="#contact"
            className="rounded-lg border border-ink-line px-6 py-3 text-sm font-semibold text-paper transition-colors hover:border-muted hover:bg-white/5"
          >
            Let's Connect
          </a>
        </div>

        <ul className="mt-10 flex flex-wrap items-center gap-2">
          {BUILDS.map((build) => (
            <li
              key={build}
              className="rounded-md border border-ink-line bg-ink-soft/70 px-3 py-1.5 font-mono text-xs text-muted"
            >
              {build}
            </li>
          ))}
        </ul>

        <div className="mt-10 flex flex-wrap items-center gap-x-8 gap-y-3 text-sm text-muted">
          <a
            href={PROFILE.github}
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-2 transition-colors hover:text-paper"
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
              <path d="M12 .5C5.65.5.5 5.65.5 12c0 5.08 3.29 9.39 7.86 10.91.58.11.79-.25.79-.55 0-.27-.01-1.17-.02-2.12-3.2.7-3.88-1.36-3.88-1.36-.52-1.33-1.28-1.68-1.28-1.68-1.04-.71.08-.7.08-.7 1.15.08 1.76 1.19 1.76 1.19 1.03 1.75 2.69 1.25 3.34.95.1-.74.4-1.25.72-1.54-2.55-.29-5.23-1.28-5.23-5.68 0-1.26.45-2.28 1.19-3.09-.12-.29-.52-1.46.11-3.05 0 0 .97-.31 3.17 1.18a11 11 0 0 1 5.77 0c2.2-1.49 3.16-1.18 3.16-1.18.63 1.59.24 2.76.12 3.05.74.81 1.18 1.83 1.18 3.09 0 4.41-2.69 5.38-5.25 5.66.41.36.78 1.06.78 2.14 0 1.54-.01 2.78-.01 3.16 0 .31.2.67.8.55A11.5 11.5 0 0 0 23.5 12C23.5 5.65 18.35.5 12 .5Z" />
            </svg>
            GitHub
          </a>
          {PROFILE.linkedin && (
            <a
              href={PROFILE.linkedin}
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-2 transition-colors hover:text-paper"
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                <path d="M20.45 20.45h-3.55v-5.57c0-1.33-.03-3.04-1.85-3.04-1.86 0-2.14 1.45-2.14 2.94v5.67H9.35V9h3.41v1.56h.05c.47-.9 1.63-1.85 3.36-1.85 3.6 0 4.27 2.37 4.27 5.45v6.29ZM5.34 7.43a2.06 2.06 0 1 1 0-4.12 2.06 2.06 0 0 1 0 4.12ZM7.12 20.45H3.55V9h3.57v11.45Z" />
              </svg>
              LinkedIn
            </a>
          )}
          <a
            href={`mailto:${PROFILE.email}`}
            className="inline-flex items-center gap-2 transition-colors hover:text-paper"
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" aria-hidden="true">
              <rect x="2.5" y="4.5" width="19" height="15" rx="2.5" />
              <path d="m3.5 6.5 8.5 6.5 8.5-6.5" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
            {PROFILE.email}
          </a>
        </div>
      </div>

      <a
        href="#about"
        aria-label="Scroll to About section"
        className="absolute bottom-6 left-1/2 hidden -translate-x-1/2 text-muted transition-colors hover:text-paper md:block"
      >
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" aria-hidden="true">
          <path d="m4 8 8 8 8-8" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </a>
    </section>
  );
}